...
@section('page_breadcrumb')
    ...
    <li class="breadcrumb-item"><a href="{{ route('barcode.batch.index') }}">Batches Barcode</a></li>
    <li class="breadcrumb-item active">Create Batch Barcode</li>
@endsection

@section('content_title', 'Create Batch Barcode')
@section('content_buttons')
    
@endsection

@section('content_body')
<form name="frmx" id="frmx" action="{{ route('barcode.batch.index') }}" method="GET">
    <div class="row">
        <div class="col-md-6">
            <div class="form-group">
                <label for="itemCode">Select Batch: <em>*</em></label>
                <select name="code" id="itemCode" class="form-control onex-select2" required="required">
                    <option value=""></option>
                    @if(!empty($batches))
                        @foreach($batches as $k => $v)
                            <option value="{{ $v->batch_no }}" @if(isset($_GET['code']) && !empty($_GET['code']) && $_GET['code'] == $v->batch_no) selected="selected" @endif>{{ $v->batch_no}} @if(!empty($v->name)) ({{ $v->name }}) @endif</option>
                        @endforeach
                    @endif
                </select>
            </div>
...
<div class="row">
    <div class="col-md-6">
        ...
        <a href="{{ route('barcode.batch.index') }}" class="btn btn-danger"><i class="fas fa-trash-alt"></i> Clear</a>
    </div>
